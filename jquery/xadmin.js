...
function video() {
    $.ajax({
        type: "POST", dataType: "json", url: "https://goms.giikin.com/admin/order/orderquery.html", data: $("#form").serialize(), success: function (result) {
            console.log(result);
            if (result.jsonCode == 0) {
                layer.msg('请粘贴订单号哦<br><img src="images/0.gif"/>', {time: 800})
            }
            if (result.jsonCode == 1) {
                layer.msg('本批次查询成功<br><img src="images/cool.gif"/>', {time: 2500});
                var html = result.con;
                $("#ullist").html(html)
            }
        }, error: function () {
            layer.msg('身份令牌失效了哦，你重新再点一次呗<br><img src="images/think.jpg"/>')
        }
    })
}
function date() {
    $.ajax({
        type: "POST", dataType: "json", url: "c/date.php", data: $("#form").serialize(), success: function (result) {
            console.log(result);
            if (result.jsonCode == 0) {
                layer.msg('请粘贴物流单号哦<br><img src="images/0.gif"/>', {time: 800})
            }
            if (result.jsonCode == 1) {
                var html = result.con;
                $("#ullist").html(html)
            ...
            layer.msg('未知错误<br><img src="images/think.jpg"/>')
        }
    })
}
function clone() {
    $.ajax({
        type: "POST", dataType: "json", url: "c/clone.php", data: $("#form").serialize(), success: function (result) {
            console.log(result);
            if (result.jsonCode == 0) {
                layer.msg('请粘贴物流单号哦<br><img src="images/0.gif"/>', {time: 800})
            }
...
function clone_old() {
    $.ajax({
        type: "POST", dataType: "json", url: "c/clone_old.php", data: $("#form").serialize(), success: function (result) {
            console.log(result);
            if (result.jsonCode == 0) {
                layer.msg('请粘贴物流单号哦<br><img src="images/0.gif"/>', {time: 800})
            }
...
function batch() {
    $.ajax({
        type: "POST", dataType: "json", url: "c/batch.php", data: $("#form").serialize(), success: function (result) {
            console.log(result);
            if (result.jsonCode == 0) {
                layer.msg('请粘贴单号哦<br><img src="images/0.gif"/>', {time: 800})
            }
            if (result.jsonCode == 1) {
                var html = result.con;
                $("#ullist").html(html)
...
document.addEventListener("visibilitychange", function () {
    if (document.visibilityState == "hidden") {
        ...
        document.title = "✪北京交通委提醒您：道路千万条 安全第一条"
    } else {
        document.title = "吉客印【很不正式】后台";
        setTimeout(function () {
...
function back() {
    $.ajax({
        ...
        success: function (result) {
            console.log(result);
            if (result.jsonCode == 0) {
                layer.msg('未提供数据<br><img src="images/0.gif"/>', {time: 800})
            }
            if (result.jsonCode == 1) {
                layer.msg("完成", {time: 500});
                var html = result.con;
            ...
            if (result.jsonCode == 2) {
                layer.msg("未查询到", {time: 1000})
            }
        },
        error: function () {
            layer.msg('密码错误or未知错误<br><img src="images/think.jpg"/>')
        }
    })
}
function login() {
...
function tuihuo() {
    $.ajax({
        ...
        error: function () {
            layer.msg('未查询到，可能是：未下单 / 后台延迟 / 参数错误 ，若检查无误，那么再点一次或者使用主接口<br><img src="images/think.jpg"/>')
        }
    })
}
function huanhuo() {
    $.ajax({
        ...
        error: function () {
            layer.msg('未查询到，可能是：权限失效 / 未下单 / 后台延迟 / 参数错误 ，若检查无误，那么再点一次或者使用主接口<br><img src="images/think.jpg"/>')
        }
    })
}
function bufa() {
...
function more() {
    var index = layer.msg('系统正在执行指令，耐心等待数据返回...', {icon: 16, time: false, shade: 0.8});
    $.ajax({
        type: "POST", dataType: "json", url: "c/more.php", data: $("#form").serialize(), success: function (result) {


            if (result.jsonCode == 0) {
                layer.msg('未粘贴数据<br><img src="images/0.gif"/>', {time: 800})
            }
            if (result.jsonCode == 1) {
                layer.close(index);
                var html = result.con;
...
function more1() {
    var index = layer.msg('系统正在执行指令，耐心等待数据返回...', {icon: 16, time: false, shade: 0.8});
    $.ajax({
        type: "POST", dataType: "json", url: "c/more1.php", data: $("#form").serialize(), success: function (result) {
